...
import { createFamilyAction, createSoloFamilyAction, joinFamilyAction } from "./actions";
...
export default function OnboardingClient() {
  ...
  async function handleSolo() {
    ...
    setLoading(true);
    try {
      const result = await createSoloFamilyAction();
      if (result?.error) {
        setError(result.error);
      } else {
        window.location.href = "/dashboard";
          ...
          {mode === "create" && (
            <div className="space-y-5">
              ...
              <button
                ...
                onClick={handleSolo}
                disabled={loading}
                className="btn-cozy btn-cozy-secondary w-full disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {loading ? "Готовим профиль..." : "Я одиночка"}
              </button>
            </div>
          )}
